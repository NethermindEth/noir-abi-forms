  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-2">
        <div className="text-zinc-100 font-medium text-sm font-mono">
          {functionArtifact.name}
        </div>
        <KindDetails
          kind={functionArtifact.type.kind}
      ...
      {(() => {
        switch (functionArtifact.type.kind) {
          case "integer":
            return (
              <IntegerTypeInput
                functionArtifact={functionArtifact.type}
                onChange={onChange}
              />
            );
          case "string":
          ...
          default:
            return (
              <div>
                Unsupported type:{" "}
                {(functionArtifact.type as { kind: string }).kind}
              </div>
            );
        }
      })()}
...
export const KindDetails: React.FC<{
  kind: AbiType["kind"];
  type: AbiType;
}> = ({ kind, type }) => {
